package timchantrongmatran2chieu;

import java.util.Scanner;

public class timchantrongmatran2chieu {
	...
	public static void TimChan(int a[][], int row , int column) {
		Boolean flag=false;
		for(int i=0;i<row;i++) {
			if(flag)
				break;
			for(int j=0;j<column;j++) {
				if(a[i][j]%2==0) {
					System.out.println("Nhần tử chẳn đầu tiền trong mảng hai chiều là: "+a[i][j]+"["+i+"]["+j+"]");
					flag=true;
					break;
				}
			}
		...
		if(!flag)
			System.out.println("Mảng không có phần tử chẳn ");
	}
